...
def normalize_excluded_paths(excluded_folder_paths: list[str]) -> set[Path]:
    return {
...
def is_excluded(path: Path, excluded_paths: set[Path]) -> bool:
...
def write_file(file_path: Path, file_text: str) -> None:
